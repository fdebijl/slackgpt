...
NO_APP_MENTION_MESSAGE = "Hi! I am not yet available to respond in this way. If you're _curious_ to use me, head to my channel under `Apps` where we can have a 1-1 chat. "
...
def respond_to_app_mention(
    ...
    logger.debug('respond to app mention')
    if not FEATURE_RESPOND_VIA_MENTION and payload.get('channel_type') != "im":
        return client.chat_postMessage(
                ...
                text=translate(
                    ...
                    context=context,
                    text=NO_APP_MENTION_MESSAGE,
                ),
            )
    
    elif payload.get('channel_type') == "im":
        return respond_in_dm(context=context, payload=payload, client=client, logger=logger)
...
def respond_in_dm(
    ...
    logger.debug('respond to dm')
    wip_reply = None
    try:
        openai_api_key = context.get("OPENAI_API_KEY")

        correct_ts = 'thread_ts' if 'thread_ts' in payload else 'ts'

        messages_in_context = client.conversations_replies(
                channel=context.channel_id,
                ts=payload[correct_ts],
                include_all_metadata=True,
                limit=1000,
            ).get("messages", [])
            ...
            messages.append(
                {
                    ...
                    "role": "user" if msg_user_id != BOT_ID else "assistant",
                }
            )
...
def respond_to_new_message(
    ...
    logger.debug('respond to new message')
